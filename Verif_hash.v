...
(*README: I worked with William Ughetta as a partner*)
...
Require Import VC.hints.  (* Import special for this tutorial. *)
...
(** A [list_cell] has four parts:
 - a linked list _cons_ cell with a key-pointer [kp], integer [count], and pointer to the [next] element of the linked list;
...
Proof. 
  intros. unfold list_cell. Exists kp. entailer!.
Qed.
(** [] *)
...
Proof.
  ...
  - unfold listrep; fold listrep.
    unfold list_cell; fold list_cell.  destruct a. entailer. entailer.
   simplify_value_fits in H2.  entailer. entailer!.
    split; intros.
...
Proof.
  intros. unfold listrep. destruct sigma eqn:Eqsigma.
  -  entailer!.
...
Lemma hashtable_rep_local_facts: forall contents p,
  ...
  intros. unfold hashtable_rep. entailer. autorewrite with sublist.
  entailer. simplify_value_fits in H1. entailer. entailer!.
  autorewrite with sublist in *|-. destruct H1. apply H1.
Qed.
...
Definition new_cell_spec : ident * funspec :=
 ...
 PRE [ _key OF tptr tschar, _count OF tint, _next OF tptr tcell ] 
   PROP()
   LOCAL(temp _key s; temp _count (Vint (Int.repr count));
         temp _next next; gvars gv)
...
Definition get_spec : ident * funspec :=
 ...
 POST [ tuint ]
    PROP ( )
    LOCAL (temp ret_temp (Vint (Int.repr (hashtable_get sigma contents))))
    SEP (hashtable_rep contents p; cstring Ews sigma s).
...
- Exists 0. entailer.
- entailer!.
- forward. forward. autorewrite with norm. deadvars!.
assert(0 <= i + 1 < Zlength (contents ++ [Byte.zero])). destruct H0.
apply Zle_lt_succ in H1. split. rep_omega. autorewrite with sublist. simpl. cstring.
forward. entailer!. cstring. entailer!. Exists (i+1). entailer!.
split.  cstring. f_equal. unfold hashfun.
assert(Znth i (contents ++ [Byte.zero]) = Znth i contents).
Search (Znth _ (_ ++ _)). apply app_Znth1. cstring.
rewrite H5. eapply hashfun_snoc. rep_omega. cstring.
- forward. entailer!. f_equal. f_equal. unfold hashfun. f_equal.
  autorewrite with sublist in *|-. assert(i = Zlength contents). cstring.
  rewrite H4.  autorewrite with sublist.  auto.
Qed.
(** [] *)
...
forward_call. forward. deadvars!. autorewrite with norm.
forward_call((tarray tschar (Zlength sigma + 1)), gv).
repeat split.
- Search (sizeof _). rewrite sizeof_tarray_tschar. rep_omega. rep_omega.
- rewrite sizeof_tarray_tschar. apply H. rep_omega.
- Intros p. deadvars!. if_tac.
  + subst. forward_if(False).
    * forward_call tt. entailer!.
    * forward. discriminate.
  + forward_if(True).
    * contradiction.
    * forward. entailer!.
    * Intros. forward_call(p, (Zlength sigma + 1), s, sigma). rep_omega.
      forward. Exists p. entailer. cancel.
      unfold cstringn. Intros. entailer. autorewrite with sublist in *|-.
      autorewrite with sublist. entailer!. unfold cstring. Intros.
      entailer!.
      Qed.
(** [] *)
...
Proof.
  start_function. forward_call((Tstruct _cell noattr), gv).
  repeat split.
  - rewrite sizeof_Tstruct. simpl. rep_omega.
  - rewrite sizeof_Tstruct. simpl. rep_omega.
  - Intros p. if_tac.
  + subst. forward_if(False).
    * forward_call tt. entailer!.
  ...
  + forward_if(True).
    * contradiction.
    * forward. entailer!.
    * Intros. forward_call(s, key, gv). Intros t'2.
      forward. forward. deadvars!. forward.
      forward. Exists p. autorewrite with norm.
      cancel. unfold list_cell; fold list_cell.
      Exists t'2. entailer!.
Qed.
(** [] *)
...
Proof.
(* use [induction] and [sep_apply]. *)
  intros. induction bl.
  - entailer!.
  -  entailer.  simpl. destruct a eqn:Ea. unfold uncurry in *. simpl in *.
     sep_apply IHbl. autorewrite with norm. Intros. entailer!.
Qed.
...
(* [rewrite (sublist_split LO MID HI) by omega], where you choose
...
*)
autorewrite with sublist. apply pred_ext.
Admitted.
(** [] *)
...
Proof.
intros.
(** use [sep_apply] with the lemmas [listcell_fold],
...
apply seq_assoc1; assert_after 1
 (EX cts:list (list (list byte * Z) * val), 
  ...
  LOCAL (temp _h (Vint (Int.repr (hashfun sigma))); temp _table table;  temp _s s) 
...
(** The purpose of this rewrite is to preserve a little 
  ...
  Hint database, the [rep_omega] tactic "knows" that N=109. *)

assert (0 <= hashfun sigma mod N < N).
  (** It is useful to put facts like this above the line, to support 
      [rep_omega] in reasoning about conversions between Z and Int. *)
  (* FILL IN HERE *) admit.
assert_PROP (Zlength cts = N) as H1.
  (** It is useful to put equations like this above the line, to support [list_solve],
      [rep_omega],  and other tactics such as [entailer!] that call them. *)
{
 (* FILL IN HERE *) admit.
}
forward.
...
(** Second, [field_at sh t gfs z p] is equivalent to
  ...
  that is, [field_address] is a way to describe
...
(** The [_next] field of [struct cell] is the 
  ...
  base of the [_next] field should be [2*sizeof(tint)]. *)
