...
Proof.
intros. induction b; try (simpl; rewrite optimize_0plus_sound; rewrite optimize_0plus_sound; reflexivity).
- reflexivity.
- reflexivity.
...
Fixpoint optimize_num (a : aexp) : aexp :=
match a with
  | ANum n => ANum n
  | APlus (ANum 0) e2 => optimize_num e2
  | APlus e1 (ANum 0) => optimize_num e1
  | APlus  e1 e2 => APlus  (optimize_num e1) (optimize_num e2)
  | AMinus e1 (ANum 0) => optimize_num e1
  | AMinus e1 e2 => AMinus (optimize_num e1) (optimize_num e2)
  | AMult _ (ANum 0) =>  ANum 0
  | AMult (ANum 0) _ =>  ANum 0
  | AMult e1 (ANum 1) =>  optimize_num e1
  | AMult (ANum 1) e2 =>  optimize_num e2
  | AMult  e1 e2 => AMult  (optimize_num e1) (optimize_num e2)
end.

Lemma optimize_num_mult : forall a1 a2,
    aeval (optimize_num a1) = aeval a1 ->
    aeval (optimize_num a2) = aeval a2 ->
    aeval (optimize_num (AMult a1 a2)) =
      aeval (AMult a1 a2).
Proof.
  intros. induction a1.
  -  induction a2.
     ...
     + destruct n.
       ...
       -- destruct n.
          ++ destruct a2_1.
             --- destruct n; try(simpl; rewrite add_0_r; simpl in H0;
                 rewrite H0; reflexivity).
             --- simpl. rewrite add_0_r. simpl in H0. rewrite H0. reflexivity.
             --- simpl. rewrite add_0_r. simpl in H0. rewrite H0. reflexivity.
             --- simpl. rewrite add_0_r. simpl in H0. rewrite H0. reflexivity.
          ++ destruct a2_1; try(simpl; simpl in H0; rewrite H0; reflexivity).
     + destruct n.
       -- reflexivity.
       -- destruct n.
          ++ simpl. rewrite add_0_r. simpl in H0. rewrite H0.
             reflexivity.
          ++ simpl. simpl in H0. rewrite H0. reflexivity.
     + destruct n.
       -- reflexivity.
       -- destruct n; try(simpl; rewrite add_0_r; simpl in H0; rewrite H0; reflexivity).
          ++ simpl. simpl in H0. rewrite H0. reflexivity.
  - destruct a2; try (simpl; simpl in H; simpl in H0; rewrite H; rewrite H0; reflexivity).
    + destruct n.
      -- simpl. rewrite mul_0_r. reflexivity.
      -- destruct n.
         ++ simpl. rewrite mul_1_r. simpl in H. apply H.
         ++ simpl. simpl in H. rewrite H. reflexivity.
  - destruct a2.
    + destruct n; try( simpl; simpl in H; simpl in H0; rewrite H; rewrite H0;
      reflexivity).
      -- simpl. rewrite mul_0_r. reflexivity.
      -- destruct n.
         ++ simpl. simpl in H. rewrite mul_1_r. apply H.
         ++  simpl. simpl in H. rewrite H. reflexivity.
    + simpl. simpl in H. simpl in H0. rewrite H. rewrite H0.
      reflexivity.
    + simpl. simpl in H. simpl in H0. rewrite H. rewrite H0.
      reflexivity.
    + simpl. simpl in H. simpl in H0. rewrite H. rewrite H0.
      reflexivity.
  - destruct a2.
    + destruct n; try( simpl; simpl in H; simpl in H0; rewrite H; rewrite H0; reflexivity).
       -- simpl. rewrite mul_0_r. reflexivity.
       -- destruct n.
          ++ simpl. rewrite mul_1_r. simpl in H. apply H.
          ++ simpl. simpl in H. rewrite H. reflexivity.
...
Theorem optimize_num_sound : forall a,
    aeval (optimize_num a) = aeval a.
Proof.
  intros. induction a; try (simpl; reflexivity).
- simpl. rewrite <- IHa1. rewrite <- IHa2.
  destruct a1 eqn:a1Eq.
  + destruct n.
    -- reflexivity.
    -- destruct a2 eqn:a2Eq; try(simpl; reflexivity).
       ++ destruct n0.
  ...
  + destruct a2 eqn:a2Eq; try(simpl; reflexivity).
    -- destruct n.
       ++ rewrite IHa2. simpl. rewrite add_0_r.
          reflexivity.
       ++ reflexivity.
  + destruct a2 eqn:a2Eq; try(simpl; reflexivity).
  ...
  + destruct a2; try(simpl; reflexivity).
     -- destruct n; try(reflexivity).
       ++ rewrite IHa2. simpl. rewrite add_0_r.
          reflexivity.
- simpl. destruct a2; try(simpl; rewrite IHa1; simpl in IHa2;
    rewrite IHa2; reflexivity).
  + destruct n; try(simpl; rewrite sub_0_r; rewrite IHa1; reflexivity).
    -- simpl. rewrite IHa1. reflexivity.
- apply optimize_num_mult.
  + apply IHa1.
  + apply IHa2.
Qed.
...
Fixpoint optimize_bool (b : bexp) : bexp :=
  ...
  | BTrue       => BTrue
  | BFalse      => BFalse
  | BEq a1 a2   => BEq (optimize_num a1) (optimize_num a2)
  | BNeq a1 a2  => BNeq (optimize_num a1) (optimize_num a2)
  | BLe a1 a2   => BLe (optimize_num a1) (optimize_num a2)
  | BGt a1 a2   => BGt (optimize_num a1) (optimize_num a2)
  | BNot BTrue     => BFalse
  | BNot BFalse     => BTrue
  | BNot b1     => BNot (optimize_bool b1)
  | BAnd (BFalse) b2  => BFalse
  | BAnd b1 BFalse  => BFalse
  | BAnd (BTrue) b2  => optimize_bool b2
  | BAnd b1 BTrue  => optimize_bool b1
  | BAnd b1 b2  => BAnd (optimize_bool b1) (optimize_bool b2)
  end.

Theorem optimize_bool_sound : forall b,
    beval (optimize_bool b) = beval b.
Proof.
  intros. induction b; try(simpl; rewrite optimize_num_sound;
    rewrite optimize_num_sound; reflexivity).
  - reflexivity.
  - reflexivity.
  - simpl. destruct b; try(simpl; rewrite optimize_num_sound;
    rewrite optimize_num_sound; reflexivity).
    + reflexivity.
    + reflexivity.
    + simpl. simpl in IHb. rewrite IHb. reflexivity.
    + simpl. simpl in IHb. rewrite IHb. reflexivity.
  - destruct b1; try(simpl; reflexivity).
    + destruct b2; try(simpl; repeat (rewrite optimize_num_sound); reflexivity).
      -- simpl. simpl in IHb2. rewrite IHb2. reflexivity.
      -- simpl. simpl in IHb2. rewrite IHb2. reflexivity.
    + destruct b2; try(simpl; repeat (rewrite optimize_num_sound); reflexivity).
      -- simpl; repeat (rewrite optimize_num_sound). Search (_ && false).
         rewrite andb_true_r. reflexivity.
      -- simpl. repeat (rewrite optimize_num_sound).
         rewrite andb_false_r. reflexivity.
      -- simpl. repeat (rewrite optimize_num_sound).
         simpl in IHb2. rewrite IHb2. reflexivity.
      -- simpl. repeat (rewrite optimize_num_sound).
         simpl in IHb2. rewrite IHb2. reflexivity.
    + destruct b2; try(simpl; repeat (rewrite optimize_num_sound); reflexivity).
      -- simpl. rewrite andb_true_r. repeat (rewrite optimize_num_sound). reflexivity.
      --  simpl. rewrite andb_false_r. repeat (rewrite optimize_num_sound). reflexivity.
      -- simpl. simpl in IHb2. rewrite IHb2. repeat (rewrite optimize_num_sound).
         reflexivity.
      -- simpl. simpl in IHb2. rewrite IHb2. repeat (rewrite optimize_num_sound).
         reflexivity.
    + destruct b2; try(simpl; repeat (rewrite optimize_num_sound); reflexivity).
    ...
    + destruct b2; try(simpl; repeat (rewrite optimize_num_sound); reflexivity).
      ...
      -- simpl. simpl in IHb2. rewrite IHb2. repeat (rewrite optimize_num_sound).
         reflexivity.
     + destruct b2; try(simpl; simpl in IHb1; simpl in IHb2; rewrite IHb1;
         rewrite IHb2; reflexivity).
      -- simpl. rewrite andb_true_r.
         simpl in IHb1. rewrite IHb1. reflexivity.
      -- simpl. rewrite andb_false_r. reflexivity.
     + destruct b2; try (simpl; simpl in IHb1; rewrite IHb1;
          repeat (rewrite optimize_num_sound);
          reflexivity).
       -- simpl. simpl in IHb1. rewrite IHb1.
          rewrite andb_true_r. reflexivity.
       -- simpl. rewrite andb_false_r. reflexivity.
       -- simpl. simpl in IHb1. rewrite IHb1.
          simpl in IHb2. rewrite IHb2.
          reflexivity.
        -- simpl. simpl in IHb1. rewrite IHb1.
          simpl in IHb2. rewrite IHb2.
          reflexivity.
...
| E_BNot (b : bexp) (tf : bool) :
  b ==>b tf ->
  BNot b ==>b negb tf
| E_BAnd (b1 b2 : bexp) (tf1 tf2 : bool) :
  b1 ==>b tf1 ->
  b2 ==>b tf2 ->
  BAnd b1 b2 ==>b  tf1 && tf2
...
Proof.
intros. split.
- intros H; induction H; subst; simpl;try apply aevalR_iff_aeval in H; try apply aevalR_iff_aeval in H0;
    try rewrite H; try rewrite H0; try reflexivity.
- generalize dependent bv. induction b; intros; simpl; subst; try constructor;
    try apply IHb; try apply IHb1; try apply IHb2;
    try apply aevalR_iff_aeval; try reflexivity.
Qed.
...
Proof.
  ...
  - apply IHcontra2. simpl. inversion Heqloopdef.
    reflexivity.
Qed.
...
| NW_Asgn (x : string) (y : aexp) : no_whilesR <{ x := y }>
| NW_Seq (c1 : com) (c2 : com)
  (H1 : no_whilesR c1)
  (H2 : no_whilesR c2) :
  no_whilesR  <{ c1 ; c2 }>
| NW_Ite (b : bexp) (ct : com) (ce : com)
    (H1 : no_whilesR ct)
    (H2 : no_whilesR ce) :
  no_whilesR (CIf b ct ce).

Theorem no_whiles_eqv:
  forall c, no_whiles c = true <-> no_whilesR c.
Proof.
intros. split.
- intros. induction c; try constructor; inversion H;
  try (apply IHc1); try (apply IHc2); try (apply andb_prop in H1);
    try (destruct H1); try (apply H0); try (apply H1).
- intros. induction c; try constructor; inversion H; subst; simpl;
  apply andb_true_iff; split; try (apply IHc1 in H0; apply H0).
  + apply IHc2 in H3. apply H3.
...
match prog with
| [] => stack
| instr :: tl => match instr with
               | SPush n => s_execute st (n :: stack) tl
               | SLoad x => s_execute st ((st x) :: stack) tl
               | SPlus => match stack with
                         | a :: b:: rest => s_execute st (a+b::rest) tl
                         | _ => s_execute st stack tl
                         end
               | SMinus => match stack with
                         | a :: b :: rest => s_execute st (b-a::rest) tl
                         | _ => s_execute st stack tl
                          end
...
Proof.
  ...
  induction p1.
  - intros. auto.
  - intros. simpl. destruct a; rewrite IHp1; auto.
    + destruct stack; auto.
     destruct stack.
         ++ auto.
         ++ rewrite IHp1. auto.
...
Proof.
  intros. generalize dependent st.  generalize dependent stack.
  induction e; intros; auto; simpl; rewrite execute_app; rewrite IHe1; rewrite execute_app;
    rewrite IHe2; simpl.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_SeqBreak (c1 c2 : com) : forall st st',
      st =[ c1 ]=> st' / SBreak ->
      st =[ c1 ; c2 ]=> st' / SBreak
  | E_SeqContinue (c1 c2 : com) : forall st st' st'' sig,
      st =[ c1 ]=> st' / SContinue ->
      st' =[ c2 ]=> st'' / sig ->
      st =[c1; c2]=> st'' / sig
  | E_IfTrue : forall st st' b c1 c2 sig1,
      beval st b = true ->
      st =[ c1 ]=> st' / sig1 ->
      st =[ if b then c1 else c2 end]=> st'/ sig1
  | E_IfFalse : forall st st' b c1 c2 sig2,
      beval st b = false ->
      st =[ c2 ]=> st' / sig2 ->
      st =[ if b then c1 else c2 end]=> st' / sig2
  | E_WhileFalse : forall b st c ,
      beval st b = false ->
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
intros. inversion H0; subst; apply E_WhileTrueBreak; assumption.
Qed.
...
Proof.
  ...
  remember <{ while b do c end }> as wcom eqn:Heq.
  induction H; intros; inversion Heq; subst.
  -  apply contra in H0; inversion H0; assumption.
  - apply IHceval2 in Heq; assumption.
...
Theorem ceval_deterministic: forall (c:com) st st1 st2 s1 s2,
     ...
     st1 = st2 /\ s1 = s2.
Proof.
  intros. generalize dependent H0. generalize dependent s2.
  generalize dependent st2. induction H; intros.
  - inversion H0; subst; split; auto.
  - inversion H0; subst; split; auto.
  - inversion H0; subst; split; auto.
  - inversion H0; subst; split; auto.
     + destruct IHceval with (st2:=st2) (s2 := SBreak); try(assumption).
     + destruct IHceval with (st2:=st'0) (s2 := SContinue); try(assumption).
       discriminate H2.
     + destruct IHceval with (st2:=st'0)(s2:=SContinue); try(assumption).
       discriminate H2.
  - apply IHceval2. inversion H1; subst.
    + destruct IHceval1 with (st2 := st2) (s2 := SBreak); try(assumption).
      discriminate H3.
    + destruct IHceval1 with (st2 := st'0) (s2 := SContinue); try(assumption); subst; try(assumption).
  - apply IHceval. inversion H1; subst; try(assumption).
    apply contra in H.
    + inversion H.
    + apply H8.
  - apply IHceval. inversion H1; subst; try(assumption).
    ...
    + inversion H.
    + apply H8.
  - inversion H0; subst; split; try (auto).
    + apply contra in H3.
      -- inversion H3.
      -- apply H.
    + apply contra in H3.
  ...
  - apply IHceval2. inversion H2; subst.
    + apply contra in H; inversion H; assumption.
    + destruct IHceval1 with (st2 := st'0) (s2 := SContinue); try assumption.
      subst; assumption.
    + destruct IHceval1 with (st2 := st2) (s2 := SBreak); try assumption.
      discriminate H4.
- inversion H1; subst.
