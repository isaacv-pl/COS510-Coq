...
Proof.
  intros. induction n.
  - simpl. reflexivity.
  - simpl. rewrite IHn. rewrite plus_n_Sm. auto.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    First, suppose [n = 0]. We must show that
    0 + m = m + 0
    From beta reduction we get that m = m + 0.
    We know that m + 0 = m from add_0_r
    ...
    from Beta reduction we get

    S (n' + m) = m + S n'
    We can use the inductive hypothesis to rewrite the eqn.

    S (m + n') = m + S n'

    We can now rewrite according to plus_n_Sm to get
...
Proof.
  ...
  rewrite add_comm with (n := n). rewrite add_assoc.
  reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite IHm. rewrite mul_n_Sm.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. rewrite add_0_r. rewrite add_0_r.
    rewrite IHb. assert (forall n m, S n + S m = S(S(n + m))).
    + intros. simpl. rewrite add_comm. simpl. rewrite add_comm.
      reflexivity.
...
Proof.
  intros. induction n.
  - simpl. reflexivity.
  - simpl. rewrite bin_to_nat_pres_incr. rewrite IHn. simpl.
    reflexivity.
Qed.
(** [] *)
...
Compute nat_to_bin (bin_to_nat (B0(B0(B0 Z)))).
Compute nat_to_bin (bin_to_nat (B1(B0(B0 Z)))).
Compute nat_to_bin (bin_to_nat (B1(B1(B0 Z)))).
...
(* Any numbers which have following B0's right before Z *)
...
Compute normalize (B1(B1(B0 Z))).
Compute normalize (B1 (B0 (B1(B1(B0 Z))))).
Compute normalize (B1 (B0 (B1(B1(B0 Z))))).
...
Theorem test : forall n, nat_to_bin (n + n)= double_bin (nat_to_bin n).
  intros. induction n.
  - simpl. reflexivity.
...
Proof.
  induction b.
  - simpl. reflexivity.
  - simpl. rewrite add_0_r. rewrite test.
    rewrite IHb. reflexivity.
  - simpl. rewrite add_0_r. rewrite test. rewrite IHb.
    unfold double_bin. destruct (normalize b).
    + simpl. reflexivity.
    + simpl. reflexivity.
    + simpl. reflexivity.
Qed.
