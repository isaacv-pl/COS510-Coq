...
Theorem de_morgan_not_or : forall (P Q : Prop),
    ~ (P \/ Q) -> ~P /\ ~Q.
Proof.
  intros. split. intro. destruct H. left. apply H0.
  intro. destruct H. right. apply H0.
Qed.
(** [] *)
...
Proof.
  intro. specialize H with (n := 0).  simpl in H.
  discriminate H.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct H.
    ...
    + destruct H. split.
      ...
      -- right. apply H0.
 - intros. destruct H.
   destruct H. left. apply H. destruct H0.
   left. apply H0. right. split.
   + apply H.
   + apply H0.
Qed.
(** [] *)
...
Proof.
  intros. intro. destruct H0 as [x e].
  specialize H with x. contradiction.
Qed.
(** [] *)
...
- induction m.
  ...
  + intros. simpl. assert (exists x : nat, m = n + x).
    apply IHn. simpl in H. apply H.
    destruct H0. exists x. simpl. apply eq_S. apply H0.
Qed.

Lemma plus_leb_inv : forall n x, (n <=? n + x) = true.
Proof.
  intros. induction n.
  - simpl. reflexivity.
  - simpl. apply IHn.
...
- induction m.
  + discriminate H.
  + rewrite H. simpl. apply plus_leb_inv.
Qed.
...
Proof.
  ...
  - intros. split.
    + simpl. intros. destruct H.
      -- left. left. apply H.
      -- simpl. Search (_ \/ _). apply or_assoc.
...
- unfold combine_odd_even. destruct odd.
  + apply H. reflexivity.
...
Proof.
  ...
  - rewrite even_S. destruct (even n) eqn:evenEq.
    + simpl. destruct IHn. exists x. f_equal. apply H.
    + simpl. destruct IHn. exists (S x). simpl. f_equal.
      apply H.
Qed.
(** [] *)
...
Proof.
intros. split.
- intros. intro. rewrite H0 in H. Search eqb. rewrite eqb_refl in H.
  discriminate H.
- intros. unfold not in H. rewrite <- not_true_iff_false.
  intro. apply H.   apply eqb_eq. apply H0.
Qed.
  (** [] *)
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  | (_ , nil) => false
  | (nil , _) => false
  | (h1 :: tl1 , h2 :: tl2) => if (eqb h1 h2) then (eqb_list eqb tl1 tl2) else false
end.
...
- generalize dependent l2. generalize dependent l1. induction l1.
  ...
  + induction l2.
    -- simpl. intros. discriminate H.
    -- intros. simpl in H. specialize l with x x0.
       destruct (eqb x x0) eqn:xEq.
       ++ destruct l. rewrite H0. f_equal.
          apply IHl1 in H. apply H. reflexivity.
       ++ discriminate H.
- generalize dependent l2. generalize dependent l1. induction l1.
  + induction l2.
    -- intros. reflexivity.
  ...
  + induction l2.
    ...
    -- intros. injection H. intros.
       simpl. apply l in H1. rewrite H1.
       apply IHl1. apply H0.
Qed.
(** [] *)
...
- intros. induction l.
  ...
  + simpl. split.
     -- simpl in H.
        destruct (test x) eqn:xEq.
        ++ reflexivity.
        ++ discriminate H.
     -- simpl. apply IHl. destruct (test x) eqn:xEq.
        ++  simpl in H. rewrite xEq in H. apply H.
        ++ simpl in H. rewrite xEq in H. discriminate H.
- intros. induction l.
  + simpl. reflexivity.
  + simpl. simpl in H. destruct H.  rewrite H. apply IHl.
    apply H0.
Qed.
...
Lemma rev_append_assoc : forall (X : Type) (l m n : list X),
    rev_append l m ++ n = rev_append l (m ++ n).
  Proof.
...
Proof.
intros. unfold tr_rev. apply functional_extensionality.
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  intros. intro. unfold not in H. apply H.
...
Proof.
intros. unfold not in H0. unfold excluded_middle in H.
specialize H with (P x). destruct H.
- apply H.
- unfold not in H. Search "ex_falso". apply ex_falso_quodlibet.
  apply H0. exists x. apply H.
  Qed.
(** [] *)
...
Proof.
  ...
  - unfold peirce. unfold double_negation_elimination.
    intros. unfold not in H. apply H. intros. apply H1.
    apply H0. specialize H with (P:= Q). intros. apply H.
...
Proof.
  ...
  - unfold double_negation_elimination. unfold de_morgan_not_and_not.
    intros. unfold not in H. unfold not in H0.
    right. apply H. intros. apply H. intros.
    apply H0. split. intros. assert(((P -> False) -> False)).  intros.
    apply H4 in H3. contradiction. admit. apply H1.
  - unfold double_negation_elimination. unfold de_morgan_not_and_not.
    intros. unfold not in H0. unfold not in H.
    assert ((P -> False) /\ (P -> False) -> False).
    intros. destruct H1. apply H0. apply H1.
    specialize H with (P:= P) (Q := P).
    assert (P \/ P). apply H. apply H1. destruct H2. apply H2. apply H2.
Admitted.
...
Proof.
  ...
  - unfold de_morgan_not_and_not. unfold implies_to_or.
    intros. unfold not. unfold not in H.
    right. apply H0. admit.
  - unfold de_morgan_not_and_not. unfold implies_to_or.
    intros. unfold not in H. unfold not in H0. simpl.
    left. Admitted.
