...
Proof.
  intros. destruct p as [x y]. simpl. reflexivity. Qed.
(** [] *)
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => O
  | hd :: tl => if (eqb hd v) then S (count v tl) else count v tl
end.
...
Fixpoint member (v : nat) (s : bag) : bool :=
  match s with
  | nil => false
  | h :: tl => if (eqb h v) then true else (member v tl)
end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => nil
  | h :: tl => if (eqb h v) then tl else h :: (remove_one v tl)
 end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h::tl => if (eqb h v) then (remove_all v tl) else h :: (remove_all v tl)
end.
...
Proof.
  intros. simpl. Search (_=?_). rewrite eqb_refl.
  reflexivity.
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | _ , nil => false
  | nil , _ => false
  | h1::tl1 , h2::tl2 => if (eqb h1 h2) then (eqblist tl1 tl2) else false
end.
...
*)
Theorem bag_count_sum : forall n l1 l2,
...
Proof.
  intros. rewrite H. rewrite <- H0. rewrite <- H.
  reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. Search (?n =? ?n). rewrite eqb_refl. reflexivity.
Qed.
(** [] *)
