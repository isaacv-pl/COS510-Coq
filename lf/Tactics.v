...
Theorem rev_exercise1 : forall (l l' : list nat),
  ...
  l' = rev l.
Proof.
  intros. rewrite H. Search rev. symmetry. apply rev_involutive.
Qed.
(** [] *)
...
(* Rewrite is a good intermediate step if the principle you are rewriting
from is not enough to solve the given goal. application is more efficient
if you are finishing and it is also good to proceed through a conditional
assumption.
...
Proof.
  intros. injection H.
  intros. assert (y::l = z::l).
  transitivity j. apply H1. apply H0.
  injection H3. intros. transitivity z. apply H2. symmetry.
  apply H4.
Qed.
(** [] *)
...
Proof.
  ...
  - (* n = S n' *) intros eq. destruct m as [| m'] eqn:E.
    + (* m = O *) discriminate eq.
    + (* m = S m' *) f_equal. injection eq. intro.
...
Proof.
  ...
  - intros. induction m.
    ...
    + simpl. f_equal.  apply IHn.
      injection H. intros. rewrite <- plus_n_Sm in H0.
      symmetry in H0.
      rewrite <- plus_n_Sm in H0. injection H0.
...
Proof.
  ...
  - intros. induction l.
    + simpl. reflexivity.
    + simpl. apply IHn. injection H. intros.
      apply H0.
Qed.
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros. destruct b eqn:HBeq.
  -  destruct (f true) eqn:HFeq.
     + rewrite HFeq. apply HFeq.
     + destruct (f false) eqn:HFFeq.
       ++ apply HFeq.
       ++ apply HFFeq.
  - destruct (f false) eqn:HFeq.
    + destruct (f true) eqn:HFTeq.
      ++ apply HFTeq.
      ++ apply HFeq.
    + rewrite HFeq. apply HFeq.
Qed.
...
Proof.
  ...
  - intros m. induction m.
    ...
    + intros. induction p.
      -- discriminate H0.
      -- simpl. simpl in H. simpl in H0. apply IHn in H0. apply H0.
         apply H.
Qed.
(** [] *)
...
Proof.
  unfold split_combine_statement. intros X Y l.
  induction l.
  - intros. induction l1.
    + induction l2.
      -- reflexivity.
  ...
  - intros. induction l1.
    ...
    + induction l2.
      --  discriminate H0.
      -- injection H0. intros. rewrite <- H2.
         rewrite <- H1. simpl. rewrite H1.
         rewrite IHl with (l1 := l1) (l2 := l2).
         reflexivity. simpl in H. Search (S _ = S _).
         apply eq_add_S in H. apply H. apply H1.
Qed.
...
Proof.
  ...
  - simpl in H. destruct test eqn:xEq.
    + injection H. intros.
      rewrite <- H1. apply xEq.
    + apply IHl. apply H.
Qed.
    (** [] *)
...
Proof.
  ...
  - destruct (test x) eqn: testEq.
    + unfold existsb'.
      simpl. rewrite testEq. simpl.
      reflexivity.
    + unfold existsb'. simpl.
      rewrite testEq. rewrite IHl. unfold existsb'.
      simpl. reflexivity.
Qed.
