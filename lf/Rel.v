...
Proof.
  ...
  assert (0 = 1) as Nonsense. {
    apply H with (x := 0).
    - unfold total_relation. left. reflexivity.
...
Proof.
unfold partial_function. intros. unfold empty_relation in *.
inversion H. inversion H0. assert (x < x).
apply PeanoNat.Nat.lt_trans with (m := y1).
apply H1. apply H2.  unfold lt in H5. Search (?x < ?x).
apply PeanoNat.Nat.lt_irrefl in H5. inversion H5.
Qed.
(** [] *)
...
Proof.
  ...
  induction o as [| o'].
- inversion Hmo.
- inversion Hmo. apply le_S in Hnm. rewrite <- H0.
  apply Hnm.
...
Theorem le_not_symmetric :
  ~ (symmetric le).
Proof.
intros. intro. unfold symmetric in H.
...
- induction b.
  + intros. inversion H.
  + intros. apply le_S_n in H , H0. apply IHa in H.
    Search (S _ = S _). apply eq_S. apply H. apply H0.
Qed.
...
- apply IHle. apply le_S. apply le_S_n in H0. apply H0.
Qed.
(** [] *)
...
- intros. apply H0.
- intros. induction H0.
  + apply rt1n_trans with y. apply Hxy. apply H.
  + apply rt1n_trans with y. apply Hxy.
    apply IHclos_refl_trans_1n.
    apply rt1n_trans with y0. apply Hxy0. apply H0.
Qed.
(** [] *)
