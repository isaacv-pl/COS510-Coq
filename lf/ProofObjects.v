...
Theorem ev_8 : ev 8.
Proof.
intros. apply ev_SS. apply ev_SS. apply ev_4. Show Proof.
Qed.
...
Theorem cf : forall P Q R, P /\ Q -> Q /\ R -> P /\ R.
Proof.
...
fun _ _ _ HC HP => match HP with
          | ex_intro _ x HPx =>
              ex_intro _ x (HC _ HPx)
          end.
...
fun _ _ _ _ _ heq teq =>
  match heq with
  | eq_refl h => match teq with
...
Proof.
intros. destruct H as [x]. apply H0.
Show Proof.
Qed.
(** [] *)
...
(fun (X : Type) (x y : X) (H : x == y) (P : X -> Prop) (H0 : P x) =>
 match H in (x0 == x1) return (P x0 -> P x1) with
 | eq_refl x0 => (fun (x1 : X) (H1 : P x1) => H1) x0
 end H0).
(** [] *)
...
Definition forward_or_distr : forall P Q R : Prop,
    P \/ (Q /\ R) -> (P \/ Q) /\ (P \/ R) :=
 fun P Q R Hor  =>
   match Hor with
...
Definition backward_or_distr : forall P Q R : Prop,
   (P \/ Q) /\ (P \/ R) -> P \/ (Q /\ R) :=
 fun P Q R Hand  =>
   match Hand with
   | conj HPQ HPR => or_elim P Q (P \/ (Q /\ R)) HPQ (fun p => or_introl p)
                      (fun q => (or_elim P R (P \/ (Q /\ R)) HPR (fun p => or_introl p)
                                (fun r => or_intror (conj q r))))
   end.
Definition or_distributes_over_and : forall P Q R : Prop,
    P \/ (Q /\ R) <-> (P \/ Q) /\ (P \/ R) :=
fun P Q R => conj (forward_or_distr P Q R) (backward_or_distr P Q R).
(** [] *)
...
Definition contradiction_implies_anything : forall P Q : Prop,
    (P /\ ~P) -> Q :=
fun _ _ Pand => match Pand with
               | conj P negP => match (negP P) with end
             end.

Definition de_morgan_not_or : forall P Q : Prop,
    ~ (P \/ Q) -> ~P /\ ~Q :=
fun P Q negPQ => conj (fun p => negPQ (or_introl p) ) (fun q => negPQ (or_intror q)).
...
Definition curry : forall P Q R : Prop,
    ((P /\ Q) -> R) -> (P -> (Q -> R)) :=
fun _ _ _ PnQtoR P Q => PnQtoR (conj P Q).

Definition uncurry : forall P Q R : Prop,
    (P -> (Q -> R)) -> ((P /\ Q) -> R) :=
fun _ _ _ PtoQtoR PnQ => match PnQ with
                      | conj P Q => PtoQtoR P Q
                      end.
...
Proof.
intros. unfold propositional_extensionality in H.
rewrite H with (P := P \/ Q) (Q := Q \/ P).
- reflexivity.
- split.
  +  intros. Search (_ \/ _). apply or_commut. apply H0.
...
Proof.
  intros. unfold proof_irrelevance. unfold propositional_extensionality in H.
  intros.  apply pe_implies_true_eq with (P := P) in H.
