...
(** **** Exercise: 1 star, standard, optional (perm)
    ...
    itself? *)
(*Example perm Perm3 nat [1;2;3] [2;1;3].*)
(* FILL IN HERE
...
Lemma perm3 : Perm3 [1;2;3] [1;2;3].
Proof.
  simpl. apply perm3_trans with (l2:=[2; 1; 3]).
  - apply perm3_swap12.
  - apply perm3_swap12.
Qed.
...
Lemma Perm3_refl : forall (X : Type) (a b c : X),
  Perm3 [a;b;c] [a;b;c].
Proof.
  intros. apply perm3_trans with (l2 := [b;a;c]).
  - apply perm3_swap12.
  - apply perm3_swap12.
Qed.
(** [] *)
...
intros. induction H0.
- simpl in H. apply H.
- simpl in H. Search ev. apply evSS_ev in H.
...
Proof.
  ...
  intros. assert (ev ((n+m) + (n+ p))).
  apply ev_sum with (n:= n+ m) (m:=n+p).
  - apply H.
  - apply H0.
  - rewrite add_comm in H1.
    assert (forall n m p, n + p + (n + m) = n + n + (m + p)).
    intros. rewrite add_comm. rewrite add_shuffle3.
    rewrite add_assoc. rewrite add_assoc. rewrite add_assoc. reflexivity.
    rewrite H2 in H1. rewrite <- double_plus in H1.
    apply ev_ev__ev with (n := (double n)) in H1. apply H1.
    Search double. apply ev_double.
Qed.
  (** [] *)
...
Proof.
  ...
  - intros. induction H.
    + apply ev'_0.
    + assert (S (S n) = 2 + n). simpl. reflexivity.
      rewrite H0. apply ev'_sum. apply ev'_2. apply IHev.
Qed.
(* *)
...
Proof.
  intros. induction H.
  - simpl. simpl in H0. destruct H0. right. left. apply H.
    destruct H. left. apply H.
    destruct H. right. right. left. apply H. destruct H.
  - simpl. simpl in H0. destruct H0. left. apply H.
    destruct H. right. right. left. apply H.
    destruct H. right. left. apply H. destruct H.
  - apply IHPerm3_2.  apply IHPerm3_1. apply H0.
Qed.
 (** [] *)
...
Lemma Perm3_NotIn : forall (X : Type) (x : X) (l1 l2 : list X),
    Perm3 l1 l2 -> ~In x l1 -> ~In x l2.
Proof.
intros. unfold not in H0. intro. apply H0.
induction H.
- simpl. simpl in H1. destruct H1. right. left. apply H.
  destruct H. left. apply H. destruct H. right. right. left. apply H.
  destruct H.
- simpl in H1. simpl. destruct H1. left. apply H. destruct H.
  right. right. left. apply H. destruct H. right. left. apply H.
  destruct H.
- apply IHPerm3_1. intros. apply H0 in H3. destruct H3.
  apply IHPerm3_2. intros. apply H0. apply Perm3_symm in H.
  apply Perm3_In with (x := x) in H.  apply H. apply H3. apply H1.
Qed.
(** [] *)
...
Proof.
  intro. apply Perm3_NotIn with (x := 4) in H.
  simpl in H. destruct H. right. right. left. auto.
  intro. simpl in H0. destruct H0.
  discriminate H0. destruct H0. discriminate H0. destruct H0.
  discriminate H0. destruct H0.
Qed.
(** [] *)
...
(** Here are a number of facts about the [<=] and [<] relations that
    ...
    practice exercises. *)
Lemma le_Sl : forall m n, S m <= n -> m <= n.
...
apply IHle. apply le_Sl in H0. apply H0.
Qed.
...
Theorem le_SS : forall m n, S m <= S n <-> m <= n.
  intros. split. intros.
    - inversion H. reflexivity.
    inversion H1. apply le_S. apply le_n.
    apply le_Sl. rewrite H3. apply H1.
...
Proof.
intros. induction b.
-  Search (_ + 0). rewrite <- plus_n_O. apply le_n.
...
Proof.
intros. split.
  - induction H. apply le_plus_l. apply le_S. apply IHle.
  - induction H. rewrite add_comm. apply le_plus_l. apply le_S. apply IHle.
Qed.
...
- left. apply le_0_n.
- intros. assert (S n + m <= p + q). apply H.  apply plus_le in H.
  destruct H. induction p.
  + simpl in H1. right. apply H1.
  + simpl in H0. rewrite le_SS in H0. apply IHn in H0.
    destruct H0.
    ++ left. rewrite le_SS. apply H0.
    ++ right. apply H0.
Qed.
(** [] *)
...
Proof.
 intros. induction H.
 -  apply le_n.
 - apply le_S in IHle. Search (S(_ + _)). rewrite <- plus_n_Sm.
   apply IHle.
Qed.
...
Proof.
  intros. induction H.
  - apply le_n.
  - apply le_S in IHle. simpl. apply IHle.
Qed.
...
Proof.
  intros. induction H.
  - Search (_ <= _ + _). apply le_plus_l.
  - apply le_S in IHle. simpl. apply IHle.
Qed.
(** [] *)

Lemma le_eq: forall a b,
a <= b -> b <= a -> a = b.
Proof.
  intros a.
  induction a.
  ...
  - induction b.
    + intros. inversion H.
    + intros. rewrite le_SS in H. rewrite le_SS in H0.
      apply IHa in H. Search (S(_) = S(_)). apply eq_S. apply H.
      apply H0.
Qed.
...
Proof.
  ...
  - induction m.
    ...
    + left. unfold lt. apply le_SS. Search (0 <= _). apply le_0_n.
  - intros. induction m.
    + right. unfold ge. apply le_0_n.
    +
Admitted.
...
intros. split.
- induction H.
  + unfold lt. apply le_SS. Search (_ <= _ + _). apply le_plus_l.
  + unfold lt. unfold lt in IHle. apply le_S. apply IHle.
- induction H.
  + unfold lt. apply le_SS. rewrite add_comm. apply le_plus_l.
  + unfold lt in *. apply le_S in IHle. apply IHle.
Qed.
(** [] *)
...
- induction m.
  ...
  + intros. simpl in H. apply le_SS. apply IHn. apply H.
Qed.
...
- intros. induction m.
  + inversion H.
  + simpl. apply IHn. rewrite le_SS in H. apply H.
Qed.
...
- intros. apply leb_correct in H. apply H.
Qed.
Theorem leb_true_trans : forall n m o,
  n <=? m = true -> m <=? o = true -> n <=? o = true.
Proof.
  intros. apply leb_complete in H. apply leb_complete in H0.
  apply leb_correct. apply le_trans with (n:= m). apply H. apply H0.
  Qed.
(** [] *)
...
(** - Which of the following propositions are provable?
      - [R 1 1 2] c4 c1 this is provable since it matches on
        +1 +1 +2 which is enought to get all three zeros

      - [R 2 2 6] This isn't provable since there is no way for
        the third number to get zero, the least it can feasibly get is
        2. for this to work m + n = o

    - If we dropped constructor [c5] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.
        No, I don't see why it would since the locations of the parameters
does not enable any different behavior, since e2 and e3 are the same.


    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.
No, it shouldn't change since c4 is essentially just c2 and c3 combined.
 *)
...
Lemma R_0_o_o : forall o, R 0 o o.
Proof.
  intros. induction o.
  - apply c1.
  - apply c3. apply IHo.
...
Lemma R_o_0_o : forall o, R o 0 o.
Proof.
  intros. induction o.
  - apply c1.
  - apply c2. apply IHo.
...
Lemma R_m_n_mn : forall m n, R m n (m + n).
Proof.
  intros. induction m.
  - simpl. apply R_0_o_o.
...
- intros. unfold fR. induction H.
  + reflexivity.
  + simpl. Search (S _ = S _). apply eq_S. apply IHR.
  + rewrite add_comm. simpl. rewrite add_comm in IHR. apply eq_S. apply IHR.
  + simpl in IHR. rewrite add_comm in IHR. simpl in IHR.
    inversion IHR. rewrite add_comm. reflexivity.
  + rewrite add_comm. apply IHR.
- unfold fR. intros. rewrite <- H. induction m.
  + induction n.
    --  simpl. apply c1.
    --  simpl. apply R_0_o_o.
  + induction n.
    -- rewrite add_0_r. apply R_o_0_o.
    -- simpl. rewrite add_comm. simpl. apply c2. apply c3.
       rewrite add_comm. apply R_m_n_mn.
Qed.
...
| s1 : subseq [] []
| s2  (hd : nat) (tl1 tl2 : list nat)   :  subseq tl1 tl2 -> subseq (hd::tl1) (hd::tl2)
| s3 (hd : nat) (l1 tl2 : list nat) :  subseq l1 tl2 -> subseq l1 (hd :: tl2)
.
...
- intros. apply H.
- intros. destruct IHsubseq (l1 := l1).


  assert (subseq l1 l3 -> subseq l1 (x :: l3)).
  intros. apply IHl2 in H. apply s3. apply H1.


intros l1 l2 l3. generalize l1 l2.  induction l3.
- intros. inversion H0. rewrite <- H1 in H. apply H.
- intros. apply IHl3 in H0. apply s3. apply IHl3 with (l2 := l4). apply H.
  apply



  induction l2.
- intros. apply subseq_empl.
-  intros. induction l3.
...
intros l1 l2. generalize l1. induction l2.
- intros. inversion H. apply subseq_empl.
- intros. apply s3 with (hd := x) in H. apply IHl2 in H.
(** [] *)
