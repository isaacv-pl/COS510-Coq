...
Definition branch_case (P : booltree_property_type) : Prop :=
  (forall (b : bool) (t1 : booltree), P t1 -> (forall (t2 : booltree), P t2
                              -> P (bt_branch b t1 t2) )).
...
(forall (x : X) P leaf x) ->
(forall (x : X) (t1 : tree X) P t1 -> (forall (t2 : tree X) P t2 -> P node t1 t2)) ->
forall t : tree X, P t
 *)
...
forall (X : Type) (P : t_tree X -> Prop),
  ...
  (forall (x : X) (t1 : t_tree X), P t1
     -> (forall (t2 : t_tree X), P t2 -> P (t_branch (t1, x, t2)))) ->
...
Definition better_t_tree_ind : better_t_tree_ind_type :=
  fun X => fun P => fun Pleaf => fun Pbr =>
    fix f (t : t_tree X) := match t with
                          | t_leaf => Pleaf
                          | t_branch (l, v, r) => Pbr v l (f l) r (f r)
                          end.
...
Theorem reflect_involution : forall (X : Type) (t : t_tree X),
    reflect (reflect t) = t.
Proof.
  intros. apply better_t_tree_ind with (t := t).
  - reflexivity.
