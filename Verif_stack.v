...
Fixpoint listrep (il: list Z) (p: val) : mpred :=
 ...
 | i::il' => EX y: val, 
        ...
        data_at Ews (Tstruct _cons noattr) (Vint (Int.repr i),y) p * listrep il' y
 | nil => !! (p = nullval) && emp
 end.
...
(** See if you can remember how to prove this; or look again
  ...
  - unfold listrep; fold listrep.
    entailer. entailer!. split; intros.
...
(** See if you can remember how to prove this; or look again
  ...
  -  entailer!.
  - Intros y. auto with valid_pointer.
Qed.
...
Lemma stack_local_prop: forall il p, stack il p |--  !! (isptr p).
  intros.
  revert p; induction il; intros p.
  - unfold stack. Intros q. entailer!.
  - unfold stack; fold stack. Intros q. entailer!.
Qed.
...
Lemma stack_valid_pointer:
  ...
  intros. unfold stack. Intros q. entailer!.
Qed.
...
Definition newstack_spec : ident * funspec :=
 ...
 POST [ tptr (Tstruct _stack noattr) ] 
    EX p: val, PROP ( ) LOCAL (temp ret_temp p) SEP (stack nil p; mem_mgr gv).
...
Definition push_spec : ident * funspec :=
 ...
 PRE [ _p OF tptr (Tstruct _stack noattr), _i OF tint ] 
    ...
    LOCAL (temp _p p; temp _i (Vint (Int.repr i)); gvars gv) 
    SEP (stack il p; mem_mgr gv)
 POST [ tvoid ] 
    PROP ( ) LOCAL () SEP (stack (i::il) p; mem_mgr gv).
...
Definition pop_spec : ident * funspec :=
 ...
 POST [ tint ] 
    PROP ( ) LOCAL (temp ret_temp (Vint (Int.repr i))) SEP (stack il p; mem_mgr gv).
...
(** An _Abstract Data Type_ (ADT) is a type provided with a 
  ...
  know the representation.  Therefore,
   ...
   At the beginning of [body_pop], of [body_push], and of
...
(* p -> top = q -> next*)
unfold listrep; fold listrep.
Intros g. forward. forward.
(* i = q -> value*)
forward.
(*free q*)
assert_PROP(y <> nullval).
{ entailer!. } forward_call((Tstruct _cons noattr), y, gv).
...
-  forward_if (True).
  + contradiction.
  + forward. entailer!.
  + Intros. forward. deadvars!. unfold stack. Intro q0.
    unfold listrep; fold listrep. Fail forward. Intros. forward.
    forward. forward. entailer. cancel.
    unfold stack. Exists q. entailer. entailer!. unfold listrep. fold listrep.
    Exists q0. entailer.
Qed.
(** [] *)
...
- forward_if (True).
  + contradiction.
  + forward. entailer!.
  + Intros. forward. forward. unfold stack. Exists p.
    Exists (Vint (Int.repr 0)). unfold listrep. entailer!.
Qed.
(** [] *)

(* 2020-04-22 08:26:22 (UTC-04) *)
